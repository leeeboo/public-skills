...
- 新增可选的 OpenWeatherMap One Call API 4.0 环境补全：仅在 FIT / COROS 温湿度缺失、不可信或明显无效，且用户明确授权后启用。
- 新增 `OPENWEATHERMAP_API_KEY` 的安全配置约定与本地辅助脚本；key 不进入聊天、仓库、命令参数或日志。
- 天气查询只发送约 `0.1°` 粗化位置和活动时间窗，不上传 FIT、GPS 或完整路线；外部天气结果不覆盖可信设备值。
- 更新复盘、隐私、README 与许可说明，区分 COROS / FIT 原始事实和 OpenWeatherMap 外部上下文。
