...
- 401 / 403 视为 key、权限或套餐问题；429 视为额度限制；5xx 或超时视为服务暂时不可用。最多重试一次，随后静默回到仅使用 COROS / FIT 的复盘路径。
...
- 保留两个来源：`FIT / COROS` 与 `OpenWeatherMap One Call API 4.0`。发现差异时报告差异，不挑一个看起来更“准确”的值覆盖另一个。
